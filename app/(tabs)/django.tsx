...
import djangoLessons from '@/data/djangoLessons.json';
...
export default function DjangoScreen() {
  const [completedIds, setCompletedIds] = useState<number[]>([]);
  const [selectedLesson, setSelectedLesson] = useState<Lesson | null>(null);

  const loadProgress = async () => {
    const ids = await getCompletedLessons('django');
    setCompletedIds(ids);
  };
  ...
  const getIconForLesson = (title: string) => {
    ...
    if (lowerTitle.includes('setup') || lowerTitle.includes('install')) return 'settings-outline';
    if (lowerTitle.includes('model') || lowerTitle.includes('database')) return 'database-outline';
    if (lowerTitle.includes('view')) return 'eye-outline';
    if (lowerTitle.includes('template')) return 'copy-outline';
    return 'layers-outline';
  };

  const globalProgress = djangoLessons.length > 0 
    ? Math.round((completedIds.length / djangoLessons.length) * 100) 
    : 0;
  ...
  const renderLessonItem = ({ item, index }: { item: Lesson, index: number }) => {
    ...
    let isLocked = false;
    if (index > 0) {
      const prevLessonId = parseInt(djangoLessons[index - 1].id, 10);
    ...
    return (
      <TouchableOpacity 
        ...
        onPress={() => isLocked ? Alert.alert('Locked', 'Please complete the previous Django lesson.') : setSelectedLesson(item)}
  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={['#00AFB9', '#0097A7']} style={styles.header}>
        <SafeAreaView>
          <View style={styles.headerTop}>
            <TouchableOpacity style={styles.backButton}>
              <Ionicons name="chevron-back" size={28} color="#FFFFFF" />
            </TouchableOpacity>
            <Text style={styles.headerTitle}>Django Expert</Text>
      ...
      <Modal visible={!!selectedLesson} animationType="slide" transparent={false}>
        <View style={styles.modalContainer}>
          <LinearGradient colors={['#00AFB9', '#0097A7']} style={styles.modalHeader}>
          ...
          {selectedLesson && (
            <ScrollView style={styles.modalScroll} contentContainerStyle={styles.modalScrollContent}>
              ...
              {selectedLesson.content.split('\n\n').map((block, idx) => {
                const isCode = block.includes('pip install') || block.includes(' = ') || block.includes('import ') || block.includes('class ');
                if (isCode) {
          ...
          {selectedLesson && (
            <View style={styles.modalFooter}>
              <TouchableOpacity 
                style={[styles.takeQuizButton, { backgroundColor: '#00AFB9' }]} 
